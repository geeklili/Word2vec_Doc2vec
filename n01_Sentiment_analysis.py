...
def load_file_and_preprocessing():
    ...
    pos = pd.read_excel('./data/pos_neg_data/pos.xls', header=None, index=None)
...
def get_train_vectors(x_train, x_test):
    ...
    # hao = w2v_model['好评']
    # print(hao)
    # w2v_model = Word2Vec(size=300, window=5, min_count=10, workers=12)
    ...
    np.save('./data/svm_data/train_vectors.npy', train_vectors)
    ...
    w2v_model.save('./data/svm_data/w2v_model.pkl')
    ...
    np.save('./data/svm_data/test_vectors.npy', test_vectors)
    print(test_vectors.shape)


def get_data():
    train_vectors = np.load('./data/svm_data/train_vectors.npy')
    y_train = np.load('./data/svm_data/y_train.npy')
    test_vectors = np.load('./data/svm_data/test_vectors.npy')
...
def svm_train(train_vectors, train_y, test_vectors, test_y):
    ...
    joblib.dump(clf, './data/svm_data/model.pkl')
...
def svm_predict(word_str):
    ...
    clf = joblib.load('./data/svm_data/model.pkl')

    result = clf.predict(words_vectors)

    if int(result[0]) == 1:
        print(word_str, ' positive')
    else:
        print(word_str, ' negative')
